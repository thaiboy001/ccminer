...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (stratum_diff != sctx->job.diff) {
		...
		applog(LOG_WARNING, "ปรับค่าความยากเป็น %g%s", stratum_diff, sdiff);
	}
	...
	while (!abort_flag) {
		...
		if (!s) {
			...
			if (!opt_quiet && !pool_on_hold)
				applog(LOG_WARNING, "การเชื่อมต่อโปรโตคอลการขุด สตราตัม ถูกขัดจังหวะ");
			continue;
		}
...
int main(int argc, char *argv[])
{
	...
	printf("สร้าง by.mantvmass\n");
	printf("ปรับแก้ by.thaiboy001\n");
	printf("แปลไทย by.thaiboy001\n");
	...
	applog(LOG_INFO, "%d Core Cpu %s เริ่ม, "
		"เรียกใช้ '%s' อัลกอริทึม.",
		opt_n_threads, opt_n_threads > 1 ? "s":"",
		pt_n_threads, opt_n_threads > 1 ? "s":"",
