			...
			json_array_foreach(txs, idx, p) {
				...
				if (work->tx_count > POK_MAX_TXS || txlen >= POK_MAX_TX_SZ) {
					...
					if (opt_debug) applog(LOG_WARNING,
						"pok: บล็อกขนาดใหญ่ละเว้น, tx len: %u", txlen);
					work->tx_count = 0;
					break;
				}
		...
		if (!have_longpoll && work->height > net_blocks + 1) {
			...
			if (opt_showdiff && net_diff > 0.) {
				if (net_diff != work->targetdiff)
					sprintf(netinfo, ", ดริฟ %.3f, พูล %.1f", net_diff, work->targetdiff);
				else
					sprintf(netinfo, ", ดริฟ %.3f", net_diff);
			}
			applog(LOG_BLUE, "%s บล็อก %d%s",
...
#define YES "ส่งงานสำเร็จ!"
#define YAY "yay!!!"
#define BOO "ส่งงานไม่สำเร็จ!"
...
int share_result(int result, int pooln, double sharediff, const char *reason)
{
	...
	if (opt_showdiff)
		sprintf(suppl, "ดริฟ %.3f", sharediff);
	...
	applog(LOG_NOTICE, "ได้รับงาน: %lu/%lu (%s), %s %s%s",
			p->accepted_count,
			p->accepted_count + p->rejected_count,
			suppl, s, flag, solved);
	if (reason) {
		applog(LOG_WARNING, "ปฏิเสธงาน: %s", reason);
		if (!check_dups && strncasecmp(reason, "ทำซ้ำ", 9) == 0) {
			applog(LOG_WARNING, "เปิดใช้งานคุณสมบัติตรวจสอบรายการซ้ำ");
			check_dups = true;
			g_work_time = 0;
		}
	}
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (pool->type & POOL_STRATUM) {
		...
		reason = json_object_get(val, "ปฏิเสธงาน-เหตุผล");
...
static bool get_mininginfo(CURL *curl, struct work *work)
{
	...
	if (!val && curl_err == -1) {
		...
		if (res) {
			json_t *key = json_object_get(res, "ความยาก");
			if (key) {
				if (json_is_object(key))
					key = json_object_get(key, "หลักฐานการทำงาน");
				if (json_is_real(key))
					net_diff = json_real_value(key);
			}
			key = json_object_get(res, "แฮชพีเครือข่าย");
			if (key && json_is_integer(key)) {
...
static bool get_upstream_work(CURL *curl, struct work *work)
{
	...
	rc = work_decode(json_object_get(val, "ผลลัพธ์"), work);

	if (opt_protocol && rc) {
		timeval_subtract(&diff, &tv_end, &tv_start);
		/* show time because curl can be slower against versions/config */
		applog(LOG_DEBUG, "ได้งานใหม่ใน %.2f มิลลิวินาที",
...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (stratum_diff != sctx->job.diff) {
		...
		applog(LOG_WARNING, "ปรับค่าความยากการขุดเป็น %g%s", stratum_diff, sdiff);
	}
...
static bool wanna_mine(int thr_id)
{
	...
	if (opt_max_diff > 0.0 && net_diff > opt_max_diff) {
		...
		if (!thr_id && opt_debug)
			applog(LOG_DEBUG, "ความแตกต่างของเครือข่ายไม่ถึงมูลค่าการดำเนินการต่อ %.3f...", opt_resume_diff);
		state = false;
	}
	// Network hashrate
	if (opt_max_rate > 0.0 && net_hashrate > opt_max_rate) {
		...
		if (!thr_id && !conditional_state[thr_id] && !opt_quiet) {
			...
			applog(LOG_INFO, "เครือข่ายแฮชเรทสูงเกินไป รออยู่ %s...", rate);
		}
		state = false;
	} else if (opt_max_rate > 0. && opt_resume_rate > 0. && conditional_state[thr_id] && net_hashrate > opt_resume_rate) {
		if (!thr_id && opt_debug)
			applog(LOG_DEBUG, "อัตราเครือข่ายไม่ถึงมูลค่าการดำเนินการต่อ %.3f...", opt_resume_rate);
		state = false;
	}
	...
	while (!abort_flag) {
		...
		if (have_stratum) {
			...
			if (secs >= scan_time || nonceptr[0] >= (end_nonce - 0x100)) {
				if (opt_debug && g_work_time && !opt_quiet)
					applog(LOG_DEBUG, "เวลางาน %u/%us nonce %x/%x", secs, scan_time, nonceptr[0], end_nonce);
				...
				if (unlikely(!get_work(mythr, &g_work))) {
					...
					if (switchn != pool_switch_count) {
						...
						applog(LOG_ERR, "การเรียกค้นงานล้มเหลว, ออกจากเทรดการขุด %d", mythr->id);
						goto out;
					}
				}
				g_work_time = time(NULL);
		...
		if (!opt_benchmark && (g_work.height != work.height || memcmp(work.target, g_work.target, sizeof(work.target))))
		{
			if (opt_debug) {
				...
				applog(LOG_DEBUG, "งาน %s เปลี่ยนเป้าหมาย: %llx (%.1f)", g_work.job_id, target64, g_work.targetdiff);
			}
		...
		if (have_stratum && work.data[nodata_check_oft] == 0 && !opt_benchmark) {
			...
			gpulog(LOG_DEBUG, thr_id, "ไม่มีข้อมูล");
			continue;
		}
		...
		if (!wanna_mine(thr_id))
		{
			...
			if (num_pools > 1 && conditional_pool_rotate) {
				if (!pool_is_switching)
					...
					if (!opt_quiet)
						applog(LOG_WARNING, "หมดเวลาเปลี่ยนพูล...");
					if (!thr_id) pools[cur_pooln].wait_time += 1;
			...
			if (shares >= opt_shares_limit) {
				...
				if (num_pools > 1 && pools[cur_pooln].shares_limit > 0) {
					if (!pool_is_switching) {
						...
						// ensure we dont stay locked if pool_is_switching is not reset...
						applog(LOG_WARNING, "เปลี่ยนเป็น พูล %d หมดเวลา...", cur_pooln);
						if (!thr_id) pools[cur_pooln].wait_time += 1;
						pool_is_switching = false;
					}
					sleep(1);
...
wait_stratum_url:
	...
	if (!pool_is_switching)
		applog(LOG_BLUE, "เชื่อมต่อพูล %s", stratum.url);
	...
	while (!abort_flag) {
			    ...
			    !stratum_authorize(&stratum, pool->user, pool->pass))
			{
				...
				if (opt_retries >= 0 && ++failures > opt_retries) {
					if (num_pools > 1 && opt_pool_failover) {
						applog(LOG_WARNING, "หมดเวลาเชื่อมต่อ Stratum, ล้มเหลว...");
						pool_switch_next(-1);
					} else {
						applog(LOG_ERR, "...สิ้นสุด งาน thread");
				...
				if (switchn != pool_switch_count)
					goto pool_switched;
				if (!opt_benchmark)
					applog(LOG_ERR, "...ลองอีกครั้งหลังจาก %d วินาที", opt_fail_pause);
				sleep(opt_fail_pause);
			}
		}
		...
		if (!stratum_socket_full(&stratum, opt_timeout)) {
			if (opt_debug)
				applog(LOG_WARNING, "หมดเวลาการเชื่อมต่อ สตราตัม");
			s = NULL;
		} else
		...
		if (!s) {
			...
			if (!opt_quiet && !pool_on_hold)
				applog(LOG_WARNING, "การเชื่อมต่อโปรโตคอลการขุดถูกขัดจังหวะ");
			continue;
		}
...
pool_switched:
	...
	if (opt_debug_threads)
		applog(LOG_DEBUG, "%s() เริ่มใหม่...", __func__);
...
void parse_arg(int key, char *arg)
{
	...
	case 'a': /* --algo */
		...
		if (i >= 0)
			...
			applog(LOG_ERR, "พารามิเตอร์อัลกอที่ไม่รู้จัก '%s'", arg);
...
static void parse_cmdline(int argc, char *argv[])
{
	...
	if (optind < argc) {
		fprintf(stderr, "%s: ไม่รองรับ-อาร์กิวเมนต์ตัวเลือก '%s' (see --help)\n",
			argv[0], argv[optind]);
		//show_usage_and_exit(1);
	}
	...
	if (opt_algo == ALGO_HEAVY && opt_vote == 9999 && !opt_benchmark) {
		fprintf(stderr, "%s: แฮชของ Heavycoin ต้องการพารามิเตอร์การโหวตรางวัลบล็อก (see --vote)\n",
			argv[0]);
		show_usage_and_exit(1);
	}
...
int main(int argc, char *argv[])
{
	...
	printf("สร้าง-miner by.Monkins1010 Twit.Monkins\n");
	printf("สร้าง-ออโต้รัน by.mantvmass Fb.Phumin Maliwan\n");
	printf("ปรับแก้-แปลไทย by.thaiboy001 Fb.Sirimekhin Suchaya\n");
	...
	if (curl_global_init(flags)) {
		applog(LOG_ERR, "เริ่มต้น CURL ล้มเหลว");
	...
	if (active_gpus == 0) {
		applog(LOG_ERR, "ไม่พบอุปกรณ์ CUDA! สิ้นสุด.");
		exit(1);
	}
	if (!opt_n_threads)
	...
	if (unlikely(pthread_create(&thr->pth, NULL, longpoll_thread, thr))) {
		applog(LOG_ERR, "สร้างเทรด longpoll ล้มเหลว");
		return EXIT_CODE_SW_INIT_ERROR;
	}
	...
	if (unlikely(pthread_create(&thr->pth, NULL, stratum_thread, thr))) {
		applog(LOG_ERR, "การสร้างเทรด stratum ล้มเหลว");
		return EXIT_CODE_SW_INIT_ERROR;
	}
	...
	if (pthread_create(&thr->pth, NULL, workio_thread, thr)) {
		applog(LOG_ERR, "สร้างเทรด งาน ล้มเหลว");
		return EXIT_CODE_SW_INIT_ERROR;
	}
	...
	if (opt_api_port) {
		...
		if (unlikely(pthread_create(&thr->pth, NULL, api_thread, thr))) {
			applog(LOG_ERR, "การสร้างเทรด API ล้มเหลว");
			return EXIT_CODE_SW_INIT_ERROR;
		}
	}
	...
	if (1) {
		...
		if (unlikely(pthread_create(&thr->pth, NULL, monitor_thread, thr))) {
			applog(LOG_ERR, "กำลังตรวจสอบเทรด %d สร้างล้มเหลว", i);
			return EXIT_CODE_SW_INIT_ERROR;
		}
	}
#endif
	...
	for (i = 0; i < opt_n_threads; i++) {
		...
		if (unlikely(pthread_create(&thr->pth, NULL, miner_thread, thr))) {
			applog(LOG_ERR, "เทรด %d สร้างล้มเหลว", i);
			return EXIT_CODE_SW_INIT_ERROR;
		}
	}

	applog(LOG_INFO, "เปิดใช้งาน ซีพียู %d เทรด",
	...
	if (opt_debug)
		applog(LOG_DEBUG, "เทรดงานหยุดทํางาน, กําลังออก.");
